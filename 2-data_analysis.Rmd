...
scores <- scores %>%
  mutate(total_design = rowSums(across(all_of(design), ~ . * weight_consensus_design[[(cur_column())]]))) %>% #creates a column for the total weighted sum scores classified as design previously
  mutate(total_methods = rowSums(across(all_of(methods), ~ . * weight_consensus_methods[[(cur_column())]]))) %>% #creates a column for the total weighted sum scores classified as methods previously
  mutate(total_reproducibility = rowSums(across(all_of(reproducibility), ~ . * weight_consensus_reproducibility[[(cur_column())]]))) %>% #creates a column for the total weighted sum scores classified as reproducibility previously
  mutate(total = rowSums(across(all_of(criteria), ~ . * weight_consensus[[(cur_column())]]))) #creates a column for the total weighted sum scores
```
